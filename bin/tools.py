...
class Run:
    ...
    @staticmethod
    def main():
        command = python_internal_ArrayImpl._get(Sys.args(), 0)
        if (command == "cp"):
            file = python_internal_ArrayImpl._get(Sys.args(), 1)
            to = python_internal_ArrayImpl._get(Sys.args(), 2)
            if sys_FileSystem.isDirectory(file):
                python_FileUtils.copyDic(file,to)
            else:
                python_FileUtils.copyFile(file,to)
...
class python_FileUtils:
    ...
    def copyFile(file,copyTo):
        if (not sys_FileSystem.exists(file)):
            haxe_Log.trace(("copyDic 路径不存在：" + ("null" if file is None else file)),_hx_AnonObject({'fileName': "src/python/FileUtils.hx", 'lineNumber': 19, 'className': "python.FileUtils", 'methodName': "copyFile"}))
            return
        startIndex1 = None
        _hx_len = None
        if (startIndex1 is None):
            ...
            i = copyTo.rfind("/", 0, (startIndex1 + 1))
    ...
    def copyDic(dic,copyTo,igone = None):
        if (not sys_FileSystem.exists(dic)):
            haxe_Log.trace(("copyDic 路径不存在：" + ("null" if dic is None else dic)),_hx_AnonObject({'fileName': "src/python/FileUtils.hx", 'lineNumber': 41, 'className': "python.FileUtils", 'methodName': "copyDic"}))
            return
        startIndex1 = None
        pos = None
        if (startIndex1 is None):
        ...
        while (_g < len(_hx_list)):
            ...
            if sys_FileSystem.isDirectory(file2):
                ...
                try:
                    ...
                    haxe_Log.trace(("Warring:" + ("null" if file2 is None else file2)),_hx_AnonObject({'fileName': "src/python/FileUtils.hx", 'lineNumber': 63, 'className': "python.FileUtils", 'methodName': "copyDic", 'customParams': [e]}))
