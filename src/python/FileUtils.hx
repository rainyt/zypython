...
class FileUtils {
	...
	public static function copyFile(file:String, copyTo:String):Void {
		...
		var dir:String = copyTo.indexOf("/") == -1 ? copyTo : copyTo.substr(0, copyTo.lastIndexOf("/"));
		if (!FileSystem.exists(dir)) {
			FileSystem.createDirectory(dir);
		}
	...
	public static function copyDic(dic:String, copyTo:String, igone:Array<String> = null):Void {
		...
		var dicName:String = dic.indexOf("/") == -1 ? dic : dic.substr(dic.lastIndexOf("/") + 1);
		if(!FileSystem.exists(copyTo + "/" + dicName))
